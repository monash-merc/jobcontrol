...
public class OAuthService {
	 ...
	 * @throws OAuthSystemException
	 */
	public static void doAuthCodeRedirect(HttpServletResponse response, URI afterAuthRedirect) throws OAuthSystemException {
		OAuthClientRequest oauthReq = OAuthService.getAuthCodeRequest(afterAuthRedirect);
		try {
	...
	public static void doTokenRequest(String code, Session session) throws OAuthSystemException, OAuthProblemException {
		String authHeader = "Basic "+new String(Base64.encodeBase64((settings.getOAuthClientId()+":"+settings.getOAuthClientSecret()).getBytes()));
		OAuthClient client = new OAuthClient(new URLConnectionClient());
		OAuthClientRequest req = OAuthClientRequest.tokenLocation(settings.getOAuthTokenEndpoint())
